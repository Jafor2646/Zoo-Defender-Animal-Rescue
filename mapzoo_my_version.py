...
habitats = [
    {"center": (-300, 300, 0), "color": (0.2, 0.7, 0.2), "name": "Savannah"},
    {"center": (300, 300, 0), "color": (0.2, 0.2, 0.7), "name": "Arctic"},
    {"center": (-300, -300, 0), "color": (0.7, 0.5, 0.2), "name": "Farm"},  # Changed from Desert
    {"center": (300, -300, 0), "color": (0.1, 0.5, 0.1), "name": "Jungle"}
]
...
def draw_environment():
    ...
    q = gluNewQuadric()
    gluSphere(q, GRID_LENGTH * 2, 20, 20)
    
    # Draw ground (large textured plane extending beyond grid)
    ground_size = GRID_LENGTH * 3
    
    glBegin(GL_QUADS)
    # Main ground
    glColor3f(*GROUND_COLOR)
    glVertex3f(-ground_size, -ground_size, -1)
    glVertex3f(-ground_size, ground_size, -1)
    glVertex3f(ground_size, ground_size, -1)
    glVertex3f(ground_size, -ground_size, -1)
    glEnd()
...
def draw_shapes():
    ...
    glColor3f(0.2, 0.4, 0.6)  # Blue pants
    gluCylinder(gluNewQuadric(), 8, 8, 20, 10, 2)
    glPopMatrix()
    ...
    # Body
    glColor3f(0, 0.7, 0.3)  # Green uniform
    glPushMatrix()
    glScalef(1, 1.2, 1.5)
    glutSolidCube(40)
    glPopMatrix()
    ...
    glColor3f(0.9, 0.9, 0.1)  # Gold badge
    glutSolidSphere(5, 8, 8)
    glPopMatrix()
    
    # Head
    glPushMatrix()  # Add push matrix for head
    glTranslatef(0, 0, 35)
    ...
    glPushMatrix()
    glTranslatef(0, 0, 10)
    glColor3f(0, 0.5, 0.2)  # Dark green hat
    glutSolidCone(16, 12, 10, 2)
    glPopMatrix()
    ...
    glRotatef(90, 0, 1, 0)
    glColor3f(0, 0.7, 0.3)  # Green uniform
    gluCylinder(gluNewQuadric(), 7, 5, 30, 8, 1)
    # Dart gun
    glTranslatef(0, 0, 30)
    glColor3f(0.4, 0.4, 0.4)
    gluCylinder(gluNewQuadric(), 8, 3, 20, 8, 1)
    glPopMatrix()
...
def mouseListener(button, state, x, y):
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        ...
        if shoot_cooldown <= current_time:
            ...
            # Calculate direction based on player angle
            angle_rad = player_angle * math.pi / 180
            direction = [math.cos(angle_rad), math.sin(angle_rad), 0]
            
            # Create new dart at player position
            dart_pos = [player_pos[0], player_pos[1], player_pos[2] + 10]
...
def setupCamera():
    ...
    if camera_mode == "third_person":
        ...
        gluLookAt(player_pos[0], player_pos[1], player_pos[2] + 40,  # Camera at player position
